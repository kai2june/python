...
dailyData["weekday"] = \
    dailyData.date.apply(lambda dateString : calendar.day_name[datetime.strptime(dateString, "%Y-%m-%d").weekday()])
print ("===dailyData['weekday']===\n",dailyData["weekday"])
dailyData["month"] = \
    dailyData.date.apply(lambda dateString : calendar.month_name[datetime.strptime(dateString, "%Y-%m-%d").month])
print ("===dailyData['month']===\n",dailyData["month"])
dailyData["season"] = dailyData.season.map({1: "Spring", 2: "Summer", 3: "Fall", 4: "Winter"})
dailyData["weather"] = dailyData.weather.map({1:"Sunny", 2:"Mist", 3:"LittleRain", 4:"HeavyRain"})
"""dailyData["weather"] = dailyData.weather.map({1:" Clear + Few clouds + Partly cloudy + Partly cloudy", \
                        2 : " Mist + Cloudy, Mist + Broken clouds, Mist + Few clouds, Mist ", \
                        3 : " Light Snow, Light Rain + Thunderstorm + Scattered clouds, Light Rain + Scattered clouds", \
                        4 :" Heavy Rain + Ice Pallets + Thunderstorm + Mist, Snow + Fog "})"""
#coerceing to category type
categoryVariableList = ["hour","holiday","workingday","weekday","month","season","weather"]
for var in categoryVariableList:
    dailyData[var] = dailyData[var].astype("category")
#drop useless column
dailyData = dailyData.drop(["datetime"], axis=1)

print ("===After dropping datetime===\n", dailyData.dtypes)
#Visualization Of DataType & Count
...
missingno.matrix(dailyData, figsize=(12,5))

#Outlier analysis
fig, axes = plt.subplots(nrows=2, ncols=3)
fig.set_size_inches(16,10)
sn.boxplot(data=dailyData, x="hour", y="count", orient="v", ax=axes[0][0])
sn.boxplot(data=dailyData, x="season", y="count", orient="v", ax=axes[0][1])
sn.boxplot(data=dailyData, x="holiday", y="count", orient="v", ax=axes[0][2])
sn.boxplot(data=dailyData, x="workingday", y="count", orient="v", ax=axes[1][0])
sn.boxplot(data=dailyData, x="weather", y="count", orient="v", ax=axes[1][1])
sn.boxplot(data=dailyData, x="temp", y="count", orient="v", ax=axes[1][2])
axes[0][0].set(xlabel="hour", ylabel="count")
axes[0][1].set(xlabel="season", ylabel="count")
axes[0][2].set(xlabel="holiday", ylabel="count")
axes[1][0].set(xlabel="workingday", ylabel="count")
axes[1][1].set(xlabel="weather", ylabel="count")
axes[1][2].set(xlabel="temp", ylabel="count")
plt.show()

fig2, axes = plt.subplots(nrows=1, ncols=3)
fig2.set_size_inches(16,5)
sn.boxplot(data=dailyData, x="atemp", y="count", orient="v", ax=axes[0])
sn.boxplot(data=dailyData, x="humidity", y="count", orient="v", ax=axes[1])
sn.boxplot(data=dailyData, x="windspeed", y="count", orient="v", ax=axes[2])
axes[0].set(xlabel="atemp", ylabel="count")
axes[1].set(xlabel="humidity", ylabel="count")
axes[2].set(xlabel="windspeed", ylabel="count")
plt.show()

#Remove outlier
dailyDataWithoutOutliers = dailyData[np.abs(dailyData["count"]-dailyData["count"].mean())<=(3*dailyData["count"].std())]
print ("===Shape Before remove outlier===\n", dailyData.shape)
print ("===Shape After remove outlier===\n", dailyDataWithoutOutliers.shape)
...
submission.to_csv('bicycle_sharing_demand_submission.csv', index=False)
